function [ maps ] = buildObjectnessMaps( img_original, ODCNN_params )
%BUILDOBJECTNESSMAP Builds the objectness probability map of an image.
    ...
    input_patch = ODCNN_params.input_patch; % size of the input patch needed for the net
    patch_props = ODCNN_params.patch_props;
    img_scales = ODCNN_params.scales;
    count_maps = 0;
    ...
    for nIS = 1:nImgScales
        ...
        stride = round(ODCNN_params.stride * img_scales(nIS));

        %% For each patch_prop
        nPatchProps = size(patch_props,1);
        for nPP = 1:nPatchProps
            count_maps = count_maps+1;
            input_img = ODCNN_params.patch_size;
            ...
            batch_images = cell(1,ODCNN_params.batch_size);
            [batch_images{:}] = deal(0);
            coord_images = zeros(ODCNN_params.batch_size, 2);
            ...
            for y = start:stride:y_end
                for x = start:stride:x_end
                    ...
                    if(nBatch == ODCNN_params.batch_size || count_computations+nBatch-1 == nComputations)
                        %% Apply classifier
                        disp(['    Evaluating ' num2str(count_computations+nBatch-1) '/' num2str(nComputations)]);
                        images = {prepare_batch2(batch_images, true, ODCNN_params.parallel)};
